...
function renderLine(line, j, accentColor){
  ...
  // ── Markdown headings: # ## ### ####
  if(/^# /.test(t)){
    // H1: 大項目 — full-width band with left border
    return(
      <div key={j} style={{
...
function ScriptViewer({content}){
  ...
  // Split into collapsible blocks on ■ PART / STEP 02-06 / 🏆
  var blocks=[];var cur=null;
  content.split("\n").forEach(function(line){
    var t=line.trim();
    var isHeader=/^■\s*PART/.test(t)||/^STEP\s+0[2-6]/.test(t)||/^🏆/.test(t);
    if(isHeader){
      if(cur)blocks.push(cur);
      var isGate=/PART\s*1|受付/.test(t);
      var isGolden=/🏆/.test(t);
            ...
            {isOpen&&(
              <div style={{background:SURFACE,padding:"12px 14px",display:"flex",flexDirection:"column",gap:7}}>
                ...
                {lines.slice(1).map(function(line,j){
                  ...
                  // Use shared renderer for everything else
        ...
        {step===6&&(<div>
          ...
          {loading&&(
            <Card style={{textAlign:"center",padding:"70px 32px"}}>
              ...
              <div style={{fontSize:13,color:TEXT_MUTED}}>3つのセクションを並列生成中（60〜90秒程度）</div>
            </Card>
          )}
          {output&&!loading&&(
            <div>
