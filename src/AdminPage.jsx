...
function DetailModal({gen,onClose,onMemoUpdate}){
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:20}}>
      <div onClick={function(e){e.stopPropagation();}} style={{background:WHITE,borderRadius:16,width:1000,maxWidth:"95%",maxHeight:"95vh",display:"flex",flexDirection:"column",boxShadow:"0 20px 60px rgba(0,0,0,0.3)",margin:"auto"}}>
        ...
        <div style={{padding:24,flex:1,overflowY:"auto",overflowX:"hidden"}}>
          {activeTab==="script"&&<div style={{whiteSpace:"pre-wrap",fontSize:13,lineHeight:1.8,color:TEXT,fontFamily:"inherit",wordBreak:"break-word"}}>{gen.output?.talkScript || "データなし"}</div>}
          {activeTab==="objection"&&<div style={{whiteSpace:"pre-wrap",fontSize:13,lineHeight:1.8,color:TEXT,fontFamily:"inherit",wordBreak:"break-word"}}>{gen.output?.objectionHandling || "データなし"}</div>}
          {activeTab==="faq"&&<div style={{whiteSpace:"pre-wrap",fontSize:13,lineHeight:1.8,color:TEXT,fontFamily:"inherit",wordBreak:"break-word"}}>{gen.output?.faq || "データなし"}</div>}
